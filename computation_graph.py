...
class Graph(object):
    ...
    def _create_trainer(self, args, g, d, loss_g, loss_d):
        ...
        with TF.control_dependencies(TF.get_collection(TF.GraphKeys.UPDATE_OPS)):
            self.grad_g = self.opt_g.compute_gradients(loss_g, var_list=g.get_trainable_weights(), colocate_gradients_with_ops=True)
            self.grad_d = self.opt_d.compute_gradients(loss_d, var_list=d.get_trainable_weights(), colocate_gradients_with_ops=True)
        if args.ggradclip:
