...
class Generator(NN.Module):
    def __init__(self,
                 ...
                 state_size=1024,
                 num_layers=1,
                 cnn_struct = [[11, 5, 512],[3, 1, 512],[11, 5, 256],[3, 1, 256],
                               [5, 2, 128],[3, 1, 128],[5, 2, 128],[3, 1, 128],[5, 2, 64],[3, 1, 1]]#,['unpool', 5, 80]
                 ):
        NN.Module.__init__(self)
...
parser.add_argument('--modelname', type=str, default = 'dataset')
...
for n_fft in [2**9, 2**10, 2**11, 2**12]:
    for win_length in [2**7, 2**8, 2**9]:
        for hop_length in [2**3, 2**4, 2**5, 2**6, 2**7]:
            start = time.time()
            dir = 'spectogram_test/' + '_'.join([str(n_fft), str(win_length), str(hop_length)]) + '/'
            try:
                os.mkdir(dir)
            except:
                pass
            for idx in range(1):
                ...
                y_abs = NP.abs(y)   # Spectrogram magnitudes
                #PL.imshow(y_abs)
                a = 2
                y_abs = y_abs + NP.random.rand(y_abs.shape[0],y_abs.shape[1]) * 1e-3
                
                # Reconstruction.  Note that only the magnitudes are used.
                w = librosa.istft(y_abs, hop_length=hop_length)
                
                for i in range(100):
                    w_ = w
                    y = librosa.stft(w, n_fft=n_fft, hop_length=hop_length)
                    z = y_abs * NP.exp(1j * NP.angle(y))
                    w = librosa.istft(z, hop_length=hop_length)
                    #print i, NP.abs(w - w_).max()
                
                librosa.output.write_wav(dir + str(idx), w, sr=8000)
            os.rename(dir[:-1], dir[:-1] + 'time:'+str(time.time() - start))
...
# Default parameters from librosa
n_fft = 2048
hop_length = 16    # Shorter hop length leads to better reconstruction but longer spectrogram
PL.close()
x = real_data[idx,:]
#x = librosa.load('/tmp/a.wav', sr=8000)[0]  # The input
x = librosa.util.fix_length(x, x.shape[0] + n_fft // 2) # See documentation of librosa.istft
y = librosa.stft(x, n_fft=n_fft, hop_length=hop_length)
y_abs = NP.abs(y)   # Spectrogram magnitudes
PL.imshow(y_abs)
PL.show()
